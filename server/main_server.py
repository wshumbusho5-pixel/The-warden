...
class InvisibleAIServer:
    ...
    async def process_request(self, request, client_id=None):
        ...
        system_prompt = (
            ...
            "answers tight."
            "\n\n"
            "FORMATTING — the client renders your response as Markdown with "
            "MathJax and syntax-highlighted code blocks. Use it well:\n"
            "- Use standard Markdown for structure: `##` / `###` headings for "
            "sections, `**bold**`, `*italic*`, `-` / `1.` for lists, "
            "`> quote` for callouts, `---` for a divider.\n"
            "- Wrap math in `$...$` (inline) or `$$...$$` (display) LaTeX. "
            "For example: `$x = \\\\frac{-b \\\\pm \\\\sqrt{b^2 - 4ac}}{2a}$`. "
            "Use it whenever equations, symbols, or fractions appear — do "
            "not spell them out in plain text.\n"
            "- Wrap code in triple-backtick fences with a language tag: "
            "```python, ```javascript, ```bash, ```json. Inline code goes in "
            "single backticks.\n"
            "- Lead with the answer, then a brief reason if useful. Drop "
